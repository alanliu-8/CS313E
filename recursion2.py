...
def groupSumClump(start, nums, target):
    ...
    indexer = start
    counter = 0
    while indexer < len(nums) and nums[start] == nums[indexer]:
        counter += nums[indexer]
        indexer += 1
    if groupSumClump(indexer, nums, target - counter):
        return True
    if groupSumClump(indexer, nums, target):
        return True
    return False
